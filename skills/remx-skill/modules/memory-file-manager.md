...
**动作流程：**
```
1. 在文件 front-matter 中设置 status: deprecated
2. 调用 remx index <file_path> --db <db_path> --meta <meta.yaml>
   → 系统自动将该记忆标记为 deprecated=1（软删除）
   → 后续检索不再出现
3. （可选）调用 remx gc --purge 物理删除已废弃记录
```
